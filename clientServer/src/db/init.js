import fs from 'fs';
import { DB_PATH, openDb, getDb, persistDb, migratePlaintextConfig } from './index.js';
...
export async function initDatabase() {
  ...
  try {
    ...
    try {
      ...
      // If migration fails due to decryption (shouldn't happen for plaintext),
    ...
    const oldResult = db._db.exec(
      `SELECT req_body_path, res_body_path FROM requests WHERE created_at < ${cutoff}`
    );
    ...
    if (oldResult.length > 0) {
      for (const row of oldResult[0].values) {
        ...
        if (reqPath && fs.existsSync(reqPath)) {
          try { fs.unlinkSync(reqPath); filesDeleted++; } catch {}
        }
        if (resPath && fs.existsSync(resPath)) {
          try { fs.unlinkSync(resPath); filesDeleted++; } catch {}
        }
      }
    }

    db._db.exec(`DELETE FROM requests WHERE created_at < ${cutoff}`);
    const changesResult = db._db.exec(`SELECT changes()`);
    const rowsDeleted = changesResult[0]?.values?.[0]?.[0] || 0;
